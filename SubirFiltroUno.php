  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
                $idFiltro = "";
                if(isset($Row[0])) {
                    $idFiltro = mysqli_real_escape_string($con,$Row[0]);
                }
                ...
                $subCategoria_id = "";
                if(isset($Row[2])) {
                  $subCategoria_id = mysqli_real_escape_string($con,$Row[2]);
                }

                if(!empty($idFiltro) || !empty($nombre) || !empty($subCategoria_id)) {
                    $query = "insert into filtroUno(idFiltro, nombre, subCategoria_id) values('".$idFiltro."','".$nombre."','".$subCategoria_id."')";
...
<div class="container">
  <h3 class="mt-5">Subir filtro uno</h3>
  <hr>
  <div class="row">
...
<?php
    $sqlSelect = "SELECT * FROM filtroUno";
