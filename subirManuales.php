  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
              ...
              $ubicacionArc = "";
              if(isset($Row[1])) {
                  $ubicacionArc = mysqli_real_escape_string($con,$Row[1]);
              }

              $marca_id = "";
              if(isset($Row[2])) {
                  $marca_id = mysqli_real_escape_string($con,$Row[2]);
              }
                
                if (!empty($nombre) || !empty($ubicacionArc) || !empty($marca_id)) {
                  $query = "insert into manuales(nombre, ubicacionArc, marca_id) values('".$nombre."','".$ubicacionArc."','".$marca_id."')";
...
<?php
    $sqlSelect = "SELECT * FROM manuales";
...
?>                  
        ...
        <tr>
          ...
          <td><?php  echo $row['ubicacionArc']; ?></td>
