  ...
  if(in_array($_FILES["file"]["type"],$allowedFileType)){
        ...
        for($i=0;$i<$sheetCount;$i++)
        {
            ...
            foreach ($Reader as $Row)
            {
              ...
              $imagen = "";
              if(isset($Row[1])) {
                  $imagen = mysqli_real_escape_string($con,$Row[1]);
              }

              $ubicacion = "";
              if(isset($Row[2])) {
                  $ubicacion = mysqli_real_escape_string($con,$Row[2]);
              }
                
                if (!empty($nombre) || !empty($imagen) || !empty($ubicacion)) {
                  $query = "insert into categoria(nombre, imagen, ubicacion) values('".$nombre."','".$imagen."','".$ubicacion."')";
